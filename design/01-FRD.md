...
- **FR-03.5** The built binaries shall carry CIRCT's assertions, and the builder shall emit, by name, the list of `obj.CIRCT` objects that do **not** reference `__assert_fail`.
  *AC:* `nm -u` over the linked `circt-opt` shows it referencing `__assert_fail`, and the emitted list of non-referencing objects is compared **as a set** against the list recorded for the previous build of the same target set at the same commit; every difference is named object by object and blocks publication until explained. The reference measurement at `5056ff04450b` is 495 of 513 referencing, so 18 objects not referencing (FINAL Appendix A), and those 18 are the baseline list. ~~The previous criterion, "at least 96% of objects", is WITHDRAWN:~~ 495/513 is 96.49% against a 96% floor, which leaves about two objects of margin, so the floor could not distinguish a drift to 20 from a broken build, and the 18 were never named.
...
- **FR-12.1** The adapter shall present a local report to the chain in the same shape the chain already consumes, so `issue_task.run_issue_remote` needs no change.
  *AC:* the chain is invoked with an unmodified `chia:examples/circt_issue_solver/issue_task.py`, asserted by a byte comparison of that file against CHIA at `16c35e92`. The chain takes `issue_md: str` and `number: int` (`issue_task.py:39`), so the adapter must supply both.
...
- **FR-14.6** Token and cost usage shall be accrued where the backend reports it.
  *AC:* the ledger's per-phase usage matches `verdict.json`'s `llm_usage` for the same phases. CHIA captures this only for the backends that report it, which are `antigravity` and `opencode` (`chia:examples/circt_issue_solver/circt_issue_loop.py:132-136`, whose own comment names them; `issue_task.py:134`). D-03 therefore defaults the campaign to `opencode`, so the field is populated; a run on the `claude` fallback records no token figure and the manifest marks that stage unmetered (FR-14.8).
...
- **FR-14.8** A stage that cannot be metered on the selected unit shall be declared unmetered in the manifest rather than silently omitted.
  *AC:* the manifest lists every stage as metered or unmetered.
...
- **NFR-08 Cost caps.** The campaign shall not exceed the budget file's per-arm allowance on the primary budget unit (G-48), nor either safety cap.
  *AC:* FR-14.4's stop and FR-18.10's stop-both-arms rule, plus a post-run reconciliation of the ledger against the cluster's own accounting. The tokens and money the campaign spends are reported beside the result and are not what the campaign is capped on (FR-14.5).
...
| A-03 | The `-UNDEBUG -gline-tables-only` image's build cost, size and speed are acceptable | **Unmeasured** | The one full-closure timing (1,027 targets, 939 s) is `-DNDEBUG` and `circt-opt` only; the `-UNDEBUG` build was 605 targets for `circt-opt` alone; and `-gline-tables-only` has never been built at all. Verify: build FR-03.7's target set under `-DNDEBUG`, under `-O3 -UNDEBUG`, and under `-O3 -UNDEBUG -gline-tables-only`, and record for each the wall time, the binary size, the image size and a per-probe slowdown. The size and build-time **delta of `-gline-tables-only` alone** is a required output of this measurement, because FR-03.4 adds it and D-08 prices it |
...
| A-11 | The SDK's `LLVMConfig.cmake` is what overrides the command-line `-DLLVM_ENABLE_ASSERTIONS=ON` | **Inference** | Three observed facts, not cmake's variable-resolution order read in this build. Verify: `cmake --trace-expand` on the configure step |
...
| A-16 | `circt-bmc --run` works against the shimmed `libz3.so.4` | **Never executed** | No requirement depends on it; recorded so the image's z3 handling is not assumed to be fully exercised |
...
| A-20 | `-UNDEBUG` leaves CIRCT's **full** lit suite no redder than `-DNDEBUG` | **Unmeasured** | FR-03.6 previously measured five directories, 523 tests. F-12's verify gate runs `circt_run_lit` over `circt_lit_gate_paths()` with CHIA's exclusions (`chia:examples/circt_issue_solver/issue_task.py:216-227`), so a test that reddens only under the flag outside those five directories makes every repair record `lit_ok=false` and FR-13.6 never return `report_plus_patch`. Verify: FR-03.6's set-equality run over the whole `test/` tree against the `-UNDEBUG` build already built at `~/.cache/chia-pin-smoke/bassert`. One command, no build. `[UNVERIFIED]` |
...
**Resolved: see `ADR/ADR-D-03-model-backend.md`.**
...
| (b) **A second `SQLiteNode` database for the loop, joined to `issues.db` by the local identifier of FR-12.2** | Clean separation; CHIA's example stays byte-identical (FR-12.1). Two files to keep on one machine (C-14). |
...
**Recommendation: (b).** It keeps CHIA's example byte-identical, which is what FR-12.1 asks for that
