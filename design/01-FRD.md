...
### 1.10 Errata from implementation, 2026-09-14/15

Dated **2026-09-15**, after §1.9 and driven by a different thing: not a decision but the code. The
driver is `reviews/implementation-errata-log.md`, the running log kept while W-05 to W-15 wrote the
modules, together with its **Architect decisions** section, which is binding and overrules any row
of the log it contradicts.

The convention is §1.5's: corrected at their own IDs, withdrawn text struck through in place, no ID
renumbered and no ID reused. `02-HLD.md` §0.4 carries the placement half, `03-LLD.md` §16.2 the full
list section by section, and `04-Test-Plan.md` §16.7 every test they changed, with §16.8 listing the
things that are defects in the **code** and are therefore owed to W-17 rather than to this document.

**One new constraint.** C-22 records GitHub's pagination ceiling, which is a fact of the environment
the design may not change and which one measurement turned from a hypothesis into a hard bound.

| ID | Change | Driver |
|---|---|---|
| FR-01.2 | The acceptance seed `f2b15a44ec70` is **not one of the 187**, its subject carrying no bug-ish word, and its `RUN:` line has **two** spaces where the criterion quotes one. Kept as the acceptance because it is the cleanest shape in the corpus, and marked as what it is: a shape example drawn from CIRCT's `test/` tree, not a member of the mined set | W-05 |
| FR-01.3 | A seed whose changed tests carry more than one `RUN:` line takes the **first** line's tool, in `test_paths` order. **27 seeds** are multi-tool. The classification is a per-seed field and M1's per-tool partition is not reconstructible from it; membership is | W-05 |
| FR-01.7 | The nearest tag and the bumps away have **no `SeedRecord` field**. They are returned as a top-level `nearest_tag` map on the builder's own return; `sdk_tag` and `bumps_away` are null on all 16 inexact seeds, which is what §2.4's own field comments already said | W-05 |
| FR-01.10 | Step 1 and step 2 are **not ordered against each other**: `env ... not ...` and `not env ...` both occur, so the rule is to strip either wrapper until neither leads. Two further measured facts of the same steps: a `;` inside a **quoted** option value is rejected by step 6 as written, conservatively, and a shell **redirect** survives normalisation as an argv token, on 10 lines across 4 seeds | W-05 |
| F-02 preconditions | ~~network access to the `llvm/circt` git remote and the GitHub releases API~~ is **WITHDRAWN**. The selector reads a **local clone only** and takes releases from `for-each-ref refs/tags/firtool-*`, never from the releases API. The fetch is the operator's, before the run | W-07 |
| F-02 acceptance | The ACs are written against the clone "at `b792c772`", which is this host's clone's **detached HEAD** and one first-parent commit behind `origin/main`. Both ACs hold at either point, the pin `e297b52ec9d8` having no release at both; the **lag differs**, 7 commits from `b792c772` and 8 from `origin/main`, and the AC now says which it is measured at | W-07 |
| FR-02.5 | "24 months" is **730 days**, measured from the **head commit's own date** and not from the day the run happens, so the window a run examines is a property of the clone. A second refusal is added: an empty tag map is `no_tags`, naming the refspec, and is not the same failure as a walk that finished without a match | W-07 |
| FR-04.6 | "prompt, streamed transcript, raw session transcript, stderr, and reported usage" is **five** files per turn and `03-LLD.md` §6.5 listed four. The prompt file is the fifth and is `llm_<stage>.prompt.md` | W-13 |
| FR-05.8 | Confirmed rather than changed, and recorded because the first implementation got it wrong: the declaration must name **Mut4All** and not merely "the design this arm reimplements". `03-LLD.md` §14.4's element row is what gained the clause | W-15 |
| FR-07.10, FR-11.3 | The prohibition binds the **renderer** and not only the agent. The report's build-identity block read "Assertions: -UNDEBUG", which puts the forbidden word in a `fatal_error` report by the loop's own hand; it now reads "Built with `-UNDEBUG`, so the compiler's internal checks are on". The literal `-UNDEBUG` stays, which is what FR-11.3 and C-08 require | W-10 |
| FR-08.1 | The `circt-opt` half is given the enumeration it never had: `_HW_TERMINAL_PASSES`, the **seven** `--lower-*-to-hw` conversions the measured source build registers, applied to the last option of the argv that is not a non-pass flag, in both dash spellings. `--convert-moore-to-core` is deliberately absent, which is this requirement's own acceptance criterion. A `--pass-pipeline=` probe is **refused** rather than parsed: **50** of M1's corpus `RUN:` lines use that spelling and **none** is HW-terminal. `firtool` with no output mode also emits Verilog by default and is excluded, this requirement saying "requesting" one of three | W-08 |
| FR-08.3 | Two of the five stimulus values, the reset protocol and the sampling point, come from the **campaign constants** of `03-LLD.md` §9.4 rather than from the `ProbeSpec`, which is the same data by this requirement's own rule that one stimulus definition serves the whole campaign | W-10 |
| FR-08.4 | "the recorded Verilator argument vector" now has a file: `differential_evidence.json`, beside the two stdout captures, carrying both arms' argv including the Verilator build's, both digests, both exit statuses, the FR-08.11 deviation, the X policy and the port-list digest | W-08 |
| FR-08.9 | The bucket is given its **observable** form, the written one being uncomputable by a differ that sees two line sequences: every signal that diverges does so over a contiguous **prefix** of the cycles it is sampled at and agrees from some cycle onward. A register the design never writes stays undefined for the whole run and is a plain `diverge`; one written at cycle k differs only while uninitialised, which is the bucket | W-08 |
| FR-08.10 | The exclusion is a **precondition of stage 6 and not a verdict inside it**. A `differential` candidate reaching the dedup stage raises rather than being recorded `dedup_unavailable`, because it is a caller defect and the verdict would hide it behind something the gate treats as ordinary | W-10 |
| FR-10.2 | The labelled set is **twenty-two pairs, not twenty**, eleven duplicate and eleven distinct, at least four per rule and five R4. **Measured: collision rate 1/11 = 0.0909 and false-merge rate 1/11 = 0.0909.** Both are printed as numerator, denominator and rate, because a small campaign gives the false-merge rate a zero denominator and a bare `0.0` would read as "no false merges" where the truth is "unmeasured" | W-10, W-15 |
| FR-10.5 | The six verdicts are given a **precedence**, which was fixed nowhere and which more than one candidate can satisfy at once: an empty mirror is `dedup_unavailable` for every candidate; then a mirror hit, open before closed; then a candidate-to-candidate duplicate; then a post-pin fix; then `new`. The mirror hit outranks the candidate duplicate because its evidence is the only carrier of FR-13.9's label | W-10 |
| FR-10.6 | The cross-run query asks for a stored fingerprint whose value matches **and whose `fingerprint_stable` is 1**. That is the only shape in which `duplicate_of_candidate` is reachable at all: FR-10.1's unstable fingerprints and the pre-re-run nulls are outside every partition, so no two candidates of one run can merge before the gate has run | W-10 |
| FR-10.7 | The detection point is recorded on the **refresh's return** and not in the manifest: `RunManifest.issue_mirror` is closed at six keys and `validate` compares the key set exactly, so `issue_mirror_refresh` returns the six plus a seventh, `incomplete_reason`, the caught exception's class name or null, and the driver copies the six | W-10 |
| FR-10.9 | The cap is a **row** bound and the walk is **two-directional**, `direction=desc` then, on the ceiling, `direction=asc`, unioned and deduplicated by number. ~~One walk at the recorded cap~~ was measured to write **zero rows**: with a token set, 100 requests in 99.3 s, the first 99 returning 200 and the hundredth **422**, GitHub's own pagination ceiling. A bounded control at a cap of 600 wrote 600 rows, 249 open and 351 closed, in 35 requests and 33.9 s. The cap stays 20,000, which two directions of 10,000 items each can now reach, `llvm/circt`'s newest issue or pull-request number on 2026-09-14 being **11,113** | W-14, architect decision 1 |
| FR-12.8 | "shall fail the run" is the **caller's** act. The adapter's exception set is closed at two refusals, so a `lit` run that discovered nothing is **recorded**, `lit_unusable` true with `lit_ok` null so that no patch can attach, and the driver stops on the field | W-14 |
| FR-13.3, FR-18.6 | A **fourth** question-2 stopping value, `not_fixpoint`, for a reducer that ran and reached no fixpoint. FR-13.3's pass requires a fixpoint and FR-18.6's table named three failing reasons, none of them this one, which `03-LLD.md` §9.5 records as the common case at the withdrawn 60 s budget. It buckets `not_minimal` with the other three, so the six buckets are unchanged and still total. A free-text `ReducedCase.reason`, which FR-09.12 permits, buckets the same way | W-14, architect decision 6 |
| FR-13.9 | The refusal is made reachable. It refuses a filing that matched **any** `good first issue` issue and the evidence block holds the labels of exactly **one**, so under the mirror screen's open-before-closed selection a labelled match could be masked by an unlabelled one and the refusal would silently not fire. The screen now promotes a labelled match ahead of the state rule, which is the safe direction for a refusal | W-10 |
| FR-13.10, FR-18.6 | A **null** answer is bucketed `undecided`. The default to `nothing` decides the outcome and FR-18.6's table had no row for an unanswered question, so the six values were not total over what the gate can produce | W-14, architect decision 6 |
| FR-13.15 | The `*Parser*.cpp` and directory halves of the check need the frame's **full path**, which `OracleVerdict.fingerprint_frame` does not carry, that field being `"<function> <basename>"`. The path is read back out of the recorded frames and re-stripped by the fingerprint's own rule; "the probe's argv carried a pass pipeline" is read off the recorded argv. Question 3 is also answered by a **node of its own**, `03-LLD.md` §3.9's `gate_validate`, the deciding stage holding no `circt` resource by FR-13.2's design | W-14, architect decision 6 |
| FR-18.2, FR-18.5 | The validation table's `probes` column counts a seed's probes across **both** arms, seeds being shared within a mode, and none of that table reaches the headline, which is FR-18.5's own point | W-15 |
| FR-18.6 | The results artefact prints a **seventh row**, `no gate decision`, which is **not** a bucket: it counts candidates the gate never reached, so a candidate is counted rather than dropped and the sum check stays meaningful. The six buckets are unchanged | W-15 |
| A-01 | A dated note rather than a status change: **six** recorded crash fixtures now exist (`crash_01`, `assertion_01` to `assertion_04`, `fatal_error_01`) across **two** SDK tags, 1.143.0 and 1.156.0, and `_ASSERT_UNREACHABLE` is still unexercised on real output. Calibration-mode build costs are measured at **14 s to 601 s per parent build**. None of that verifies the assumption, which needs F-04's pilot | W-09 |
| A-05 | **Closes** on two measured numbers, the collision rate 1/11 and the false-merge rate 1/11 over the 22 labelled pairs, with the caveat printed beside them: the set is constructed from **six recorded failures** and not from a pilot's own probes, which `04-Test-Plan.md` §10 asks for and which no run has produced | W-10 |
| A-08 | **Closed by measurement.** Both harness generators work from a port list: the clocked accumulator agrees over all **56** sampled cycles through `oracle_differential` end to end, and the same design with one constant changed diverges at line index 1, cycle 8, signal `p`, `1` against `0` | W-08 |
| A-19 | **Still open, and now unblocked.** FR-08.1's rule exists as a pure function; the count over the 187-seed corpus has not been run. `[UNVERIFIED]` | W-08 |
| C-22 | **New.** GitHub's issues-listing endpoint refuses page 100, that is item 10,001, with a **422** | W-14, architect decision 1 |
...
- **FR-01.2** The builder shall extract, for each seed, every `RUN:` line of each changed test file, verbatim, and the first CIRCT tool name each line invokes. The verbatim line is kept; FR-01.10 normalises a copy of it into the argument vector a `ProbeSpec` carries.
  *AC:* for seed `f2b15a44ec70` the extracted line is `circt-opt %s --split-input-file --convert-hw-to-llvm=spill-arrays-early=false | FileCheck %s` and the tool is `circt-opt` (PIN §5.3). That is the cleanest shape in the corpus; FR-01.10's acceptance exercises the rest. **Amended 2026-09-15 (§1.10):** `f2b15a44ec70` is **not one of the 187**, its subject carrying no bug-ish word, so this is a shape example drawn from CIRCT's `test/` tree and not a member of the mined set; and the recorded line has **two** spaces where this criterion quotes one, so the comparison is on the normalised line.
- **FR-01.3** The builder shall classify each seed's entry tool as one of `circt-opt`, `firtool`, `circt-verilog`, `circt-translate`, `arcilator`, `other`, taken from FR-01.2's tool name, never inferred from the source path.
  *AC:* every one of the 187 records has a classification; the count of `other` is reported and the records carrying it are listed by SHA. **Amended 2026-09-15 (§1.10):** where a seed's changed tests carry more than one `RUN:` line, the classification is the **first** line's tool in `test_paths` order; **27 seeds** are multi-tool, and their per-tool partition is not reconstructible from a per-seed field, though membership is.
...
- **FR-01.7** The builder shall mark a seed `sdk_exact=false` when no tag matches, and shall record the nearest tag by date and the number of LLVM bumps away. **Amended 2026-09-15 (§1.10):** those two values have **no `SeedRecord` field** and are recorded on the builder's own return as a top-level `nearest_tag` map; `sdk_tag` and `bumps_away` are null on exactly the 16 inexact seeds, which is what `03-LLD.md` §2.4's field comments already said. A schema field for a reporting figure would have been a contract MAJOR bump.
  *AC:* 16 seeds carry `sdk_exact=false`, all with bumps-away equal to 1, and none greater (PIN §4), the two values read from `nearest_tag`.
...
- **FR-01.10** The builder shall normalise every extracted `RUN:` line into a tool plus an argument vector by exactly these steps, in this order, recording what each step did: (1) strip a leading `not` wrapper and record `polarity=expect_nonzero`, otherwise `polarity=expect_zero`; (2) strip a leading `env` wrapper, recording the environment assignments it carried; **steps 1 and 2 are not ordered against each other (amended 2026-09-15, §1.10): both `env ... not ...` and `not env ...` occur, so either wrapper is stripped until neither leads**; (3) strip a leading `split-file` wrapper and record the seed as `shape=split_file`; (4) drop everything from the first unquoted `|` onwards, which removes the `| FileCheck %s` tail and any other downstream filter; (5) resolve lit's substitutions as lit resolves them, `%s` to the input path, `%t` to a per-probe temporary path, `%S` to the test file's directory; (6) mark any line still carrying a shell construct after steps 1 to 5, that is a `;`, a `&&`, a backtick, a `$(`, or a `%{...}` substitution, as `shape=unsupported`. A seed whose line is `shape=unsupported` shall be excluded from both arms and counted.
  *AC:* the builder reports, over the corpus, the count of each `polarity`, the count of each `shape`, and the count of excluded seeds; and a unit test covers one line of each shape, including one `not`-wrapped line and one `split-file` line. **Two measured limits of step 6, recorded 2026-09-15 (§1.10) rather than fixed:** a `;` inside a **quoted** option value is rejected, conservatively, the step being a scan that does not track quoting, and a shell **redirect** survives normalisation as an argv token, on **10 lines across 4 seeds**. Rejecting is the safe direction: an unsupported seed is excluded and counted, a mis-parsed one runs the wrong command. RT4 §2 W2 measured over CIRCT `test/` at `b792c772`: 1,714 `RUN:` lines, 81 beginning with `not`, 1,302 containing a pipe, 218 using `%t`, `%S` or `%{...}`, 11 using `split-file`; those are RT4's measurements and this requirement is what makes them actionable.
...
releases API~~ **a local blobless `llvm/circt` clone whose `firtool-*` tags have been fetched**.
**WITHDRAWN 2026-09-15 (§1.10):** the selector uses no network at all. It reads the clone only, and
takes releases from `for-each-ref refs/tags/firtool-*`, never from the releases API; the fetch is the
operator's, before the run.
...
- **FR-02.5** When no first-parent commit within the last 24 months has a matching release, the selector shall exit non-zero and name the last window it examined, rather than widening its search. **Amended 2026-09-15 (§1.10):** "24 months" is **730 days**, measured from the **head commit's own date** and not from the day the run happens, so the window is a property of the clone and a re-run a week later examines the same one, which is what FR-01.6 needs. And the refusal is **two**, not one: an empty tag map is `no_tags`, naming the refspec and raised before the walk starts, because a clone fetched without `--tags` is a different operator action from a `main` that has drifted.
  *AC:* a unit test with an empty tag list exits non-zero **naming the refspec `refs/tags/firtool-*`**; a unit test whose walk finishes with no matching release exits non-zero naming the last window examined.
...
Appendix A). **Amended 2026-09-15 (§1.10):** `b792c772` is this host's clone's **detached HEAD** and
is one first-parent commit behind `origin/main`. The criterion holds at either point, the pin having
no release at both; the **lag differs**, 7 commits from `b792c772` and 8 from `origin/main`, so the
acceptance states which ref it was measured at, and the acceptance run passes `origin/main`.
...
- **FR-04.6** Every stage-1 and stage-2 turn shall be persisted in full: prompt, streamed transcript, raw session transcript, stderr, and reported usage. That is **five** files per turn, and `03-LLD.md` §6.5 listed four until 2026-09-15 (§1.10); the prompt is the fifth and is `llm_<stage>.prompt.md`.
...
**Erratum 2026-09-15: the shape of a stage-1 and a stage-2 turn.** A turn's tool budget is **three
phases inside the backend**, not one loop. Phase A is `max_tool_iterations` calls with every tool
callable. Phase B runs for **stage 2 only**, is `per_seed_probe_cap + 1` calls in function-calling
mode `ANY` restricted to the probe-writing method, and exists because stage 2's deliverable is itself
a tool call: a turn that spent its reading budget would otherwise have no call left in which to
write. Phase C follows unconditionally and is one **tool-free** call in mode `NONE`, carrying a user
message that says the tool calls are exhausted and asks for the answer in exactly the required
format. The tool **declarations** stay in that request and only the mode changes, because dropping
them from a conversation whose history holds function-call parts leaves the request inconsistent and
the model answers it with no part at all. Without phase C a turn that spent its whole budget reading
returned an empty result, failed the output contract of FR-04.1 and FR-04.2 and wrote no probing
input at all, which is what a pilot measured on both of its seeds. The read-only source tool of
FR-04.4 therefore **pages**: one `read_file(path, first_line)` returns 64 KiB and ends in a marked
tail naming the line to continue from and the file's length, and the `grep` method pages the same way
and says how many matches it dropped. Both prompts state their own budgets, the tool-call cap and,
for stage 2, the write-call cap. A rate-limit refusal is waited out **inside** the call rather than
ending the turn: exponential backoff from 20 s, doubling to a cap of 300 s, six tries, jittered, with
a delay the refusal itself names taking precedence. A refused call is billed nothing, so waiting
costs wall clock only, which the arm window already bounds.

**Erratum 2026-09-15: four further corrections to the turn, and they are what made iterations 2 and
3 worth paying for.** (a) The streamed transcript of FR-04.6 is written in a `finally`. A turn that
raised left a zero-byte transcript, so the one record of what it did died with the exception and the
failure could not be diagnosed at the very moment FR-04.8 says to record its cause. (b) A response
truncated at the output cap **with complete tool calls in it** now has those calls executed and the
loop continued; only a truncation carrying no call ends the turn, that being the case with nothing
to go on with. (c) Stage 2's output cap is doubled over every other stage's, it alone writing
several whole programs and its json footer in one response and having been truncated on a campaign's
first seed; the ceiling FR-14.6 prices is the larger one, so the money follows the cap rather than
discovering it. (d) Stage 1's answer does not change between the iterations of one seed, and
re-sending its turn cost USD 1 to 1.5 a time on large seeds: it is made **once per seed** and reused
on iterations 2 and 3, the reused iteration writing its usage file with the reuse recorded in it so
that FR-04.6's five files per turn stay countable, and a flag forces the re-read. The requirement's
own per-iteration accounting is unaffected: a reused turn is charged nothing because nothing was
sent.
...
**Erratum 2026-09-15: a seed whose own test its entry tool no longer accepts.** Before any mutator
runs, the arm runs the seed's own **unmutated** first test file through its entry tool at the run's
commit, under F-06's per-probe limits. An ordinary non-zero exit means every mutant of that seed
would inherit the rejection and the arm would learn nothing from it, so the seed is recorded
`stale_at_build` and skipped: no probing input is written, no mutator is charged, and the tool's own
stderr is kept beside the input it refused. A **signal** or a limit hit is a finding and not
staleness, and does not skip the seed. Separately, FR-05.5's preserved argument vector can never
carry `-verify-diagnostics` into a mutant: the vector the arm preserves is the seed's template with
the probe-only options already stripped, in both dash spellings, so a mutator whose pattern targets
that option cannot fire in the arm at all.
...
- **FR-06.9** `BuildResult.status` shall take exactly one of seven values and no other: `clean_exit`, `parse_error`, `assertion`, `fatal_error`, `crash`, `timeout`, `oom`.
  ...
  **Erratum 2026-09-15 (contracts 2.1 and 2.4): the list is nine, not seven.** `tool_unavailable` was added at contract 2.1 for a probe whose tool could not be loaded, and this requirement never carried it; `verifier_error` is added at contract 2.4 for a probe the tool refused **its own output** on, which this list booked as `parse_error` and so charged to the loop rather than to the compiler. F-07's erratum carries the rule that decides it and `03-LLD.md` §3.6 the patterns; `OracleClass` gains the same value and is **five**. The feature acceptance above is unchanged and still produces the seven it names.
...
- **FR-07.10** A `fatal_error` firing shall be reported in wording that names it as a refusal the compiler chose, not as a crash: the report shall quote the `LLVM ERROR:` message and shall state that the input reached a path CIRCT declines to compile, and it shall not use the words "crash" or "assertion" for it. **Amended 2026-09-15 (§1.10): the prohibition binds the renderer and not only the agent.** The report's build-identity block read "Assertions: -UNDEBUG", which put the forbidden word into a `fatal_error` report by the loop's own hand; it reads "Built with `-UNDEBUG`, so the compiler's internal checks are on". The literal `-UNDEBUG` stays, which is what FR-11.3 and C-08 require.
...
**Erratum 2026-09-15 (D-13): a fourth firing class, `verifier_error`, for the compiler refusing its
own output.** MEASURED, campaign 2: ten probes of one seed ran `circt-opt --convert-moore-to-core`
and exited 1 with `error: 'comb.extract' op result #0 must be a signless integer bitvector, but got
'!hw.array<2xi2>'`, and with the `llhd.sig.extract` equivalent over an aggregate ref. Neither op is
in the input. The conversion pass created them with aggregate types and MLIR's verifier refused the
compiler's own output, which is a CIRCT defect of exactly the kind this feature exists to find, and
FR-06.6 booked every one of them as `parse_error`: the oracle did not fire, the taxonomy charged the
failure to the loop, and F-16's feedback told the model to fix an input that was fine. The oracle
now fires, as `verifier_error`, when **both** conditions hold and neither alone. The probe's first
`error: '<op>' op ...` diagnostic has MLIR's **generated** verifier grammar,
`<operand|result|region|successor> #N must be ...` or `... failed to satisfy constraint: ...`, which
is the shape that generated verifier writes and no hand-written diagnostic does; and FR-13.15's
parse-and-verify command for the input's own language, run on the **input alone** at the run's
commit on the same worker, exits 0, which says the input is valid and the failure is the tool's. The
second conjunct alone is measured to be insufficient: recorded mutation probe `p-b3831e4112d4` is
`circt-opt -lower-handshake-to-hw` on an input the loop fed without `--split-input-file`, and it
exits 1 with `'builtin.module' op multiple candidate top-level modules detected ...` on an input
that parses and verifies, a pass complaining about how it was invoked. The class fires no
`out_of_scope_root`, a verifier diagnostic carrying no stack trace to root, and FR-07.5's exclusion
is untouched. Downstream: the fingerprint is the op, the message with every quoted op name and type
generalised, and the pass the argv named, so one invariant broken on one op with two different
aggregate types is one candidate (FR-10.1); the reducer's interestingness test compares the same
pair and never the concrete type, which is what lets reduction change it (FR-09.3); gate question 3
reads the parse-and-verify run stage 3 already made rather than asking a fourth time, and passes by
construction (FR-13.15); repair does **not** take the class, its two admitted classes being `crash`
and `assertion`; and the feedback bundle carries the probe as a **finding**, telling the next turn
not to repeat or repair the input (FR-16.1). Every stored probe of every run can be re-judged under
the rule without a model turn, by `bug_loop.py --reclassify <run>`; over the nine stored runs it
moved 11 `parse_error` probes to `verifier_error`, all of one seed, and 0 of the mutation arm's 400.
...
- **FR-08.1** The differential shall be attempted only for probes whose entry tool can produce both HW-dialect IR for arcilator and Verilog for Verilator, that is `firtool` and `circt-opt` probes reaching the HW dialect; every other probe shall record `not_applicable` with the reason. Applicability is decided from the entry tool and the requested output mode **before** either simulator runs; a design a simulator then rejects is `harness_failure` (FR-08.8), never `not_applicable`.
  Applicability is decided by a rule, stated here and nowhere else: the seed's entry tool, run with the probe's requested output mode, produces both HW-dialect IR arcilator accepts and synthesisable Verilog Verilator accepts. That admits `firtool` probes requesting `--ir-hw`, `--verilog` or `--split-verilog`, and `circt-opt` probes whose pipeline ends in the HW dialect; it admits nothing else. **Amended 2026-09-15 (§1.10): "a pipeline that ends in the HW dialect" is given the enumeration it never had**, without which the `circt-opt` half is not a testable rule. It is the **seven** `--lower-*-to-hw` conversions the measured source build registers (`circt-opt --help`, 2026-09-14), matched against the **last** option of the argv that is not a non-pass flag, in both dash spellings. `--convert-moore-to-core` is deliberately absent, which is this requirement's own acceptance criterion. A probe spelling its pipeline as `--pass-pipeline=` is **refused rather than parsed**: **50** of M1's corpus `RUN:` lines use that spelling and **none** of them is HW-terminal, so reading the last pass out of a nested pipeline string would be code for no case. `firtool` with **no** output mode also emits Verilog by default and is excluded, this requirement saying "requesting" one of three.
...
- **FR-08.3** Both harnesses shall use one shared stimulus definition, one reset protocol and one sampling point, all declared in the `ProbeSpec` and recorded in the verdict.
  *AC:* the recorded stimulus, reset and sampling fields are identical for the two arms of one comparison; a unit test asserts equality. **Amended 2026-09-15 (§1.10):** the reset protocol and the sampling point are **campaign constants** (`03-LLD.md` §9.4) rather than `ProbeSpec` fields, which is the same data by this requirement's own rule that one stimulus definition serves the whole campaign; the report renderer reads them from there, having no `ProbeSpec`.
- **FR-08.4** The X-initialisation and reset policy shall be explicit, and Verilator shall be invoked with `--x-initial` and `--x-assign` set to the declared policy.
  *AC:* the recorded Verilator argument vector contains both flags with the policy's values (FINAL Appendix A records the flags exist in Verilator 5.052), and the verdict carries the Verilator version FR-03.15 pins for the campaign. **Amended 2026-09-15 (§1.10): the recorded argument vector now has a file.** `differential_evidence.json` sits beside the two stdout captures and carries both arms' argv, the Verilator **build's** included, both stdout digests, both exit statuses, the FR-08.11 driver deviation, the X policy and the port-list digest; `03-LLD.md` §6.5 named no file that held one. The flags are the visible half of the risk; the invisible half is that X propagation itself differs between Verilator versions, which is why the version is fixed for the campaign rather than merely recorded.
...
- **FR-08.9** A divergence confined to signals the declared X policy leaves undefined shall be recorded as `diverge_x_policy` and excluded from the candidate count. **Amended 2026-09-15 (§1.10):** ~~"confined to cycles before the first write of the divergent output signal"~~ is **WITHDRAWN** as uncomputable by a differ that sees two line sequences and no write history. Its observable form is the rule: every signal that diverges does so over a contiguous **prefix** of the cycles it is sampled at, and agrees from some cycle onward. A register the design never writes stays undefined for the whole run under `--x-initial unique` and is a plain `diverge`; one written at cycle k differs only while uninitialised, which is what this bucket is for.
  *AC:* a probe reading a register before any clock edge, under a policy that declares that undefined, records `diverge_x_policy`; a probe whose divergence is not a prefix records `diverge`.
...
- **FR-10.2** Two candidates shall be declared duplicates **if and only if** their primary fingerprints are equal, compared as exact strings or as exact equality of the ordered N-name tuple. No similarity measure, no threshold and no other key participates. The structural hash and the frame tuple are recorded beside the verdict as evidence and shall never merge two candidates.
  *AC:* a test set of at least 20 `[DEFAULT]` hand-labelled duplicate and non-duplicate pairs yields a reported collision rate and a reported false-merge rate, both printed in the results artefact beside the headline they qualify; and a test asserts that the duplicate relation is reflexive, symmetric and transitive, which it is by construction from string equality. ~~**Neither rate is known today** (A-05)~~ **Measured 2026-09-15 (§1.10):** the labelled set is **twenty-two** pairs, eleven duplicate and eleven distinct, and both rates are **1/11 = 0.0909**. Each is printed as numerator, denominator and rate, because a small campaign gives the false-merge rate a **zero denominator** and a bare `0.0` would read as "no false merges" where the truth is "unmeasured". The acceptance is still that the rates are measured and reported, not that they meet a threshold. ~~The rule this replaces, "duplicates when **any one** of the three keys is equal",~~ was not transitive: A could share an assertion with B and B a structural hash with C while A and C shared nothing, so pairwise marking gave two bugs and a union-find closure gave one, and the paper's headline differed between two faithful implementations of the same requirement. One key, one equivalence relation, one partition.
...
- **FR-10.4** The stage shall scan commits made on `main` after the run's commit (FR-02.7) and shall record a match as `fixed_post_pin` when a commit touches a source file named in the candidate's symbolised frames, or touches the file named in the candidate's assertion `file:line`. This rule matches at **file** level, and file level is defensible **here and only here**: the lower bound is the run's commit, so the window is the lag, at most 100 commits and 16.1 days (FINAL Appendix A), over which a small share of the tree moves. It is over-inclusive by design, because a match makes the candidate fail the gate's novelty question, so a false match costs a report rather than a wrong filing. FR-15.1 does **not** inherit the file-level rule; its window is 24 months, where file level is near-universal, and it uses symbol level instead.
  ...
  **Erratum 2026-09-15 (D-11): the match level is symbol, and a file touch is recorded rather than acted on.** MEASURED, campaign 2: the first real assertion the loop found, `!values.empty() && "Cannot build array of zero elements"` at `HWOps.cpp:1995`, was screened `fixed_post_pin` because commit `ddb3d1bd`, "Bump LLVM" over 87 files, touched `HWOps.cpp` and `MooreToCore.cpp` at hunks having nothing to do with it, while FR-15.1's symbol-level scan over the same frames reported no contamination at all. A candidate refused at gate question 4 over an LLVM bump is a bug the campaign found and discarded, so this rule matches at **symbol** level, by the same test FR-15.1 uses. The file-level matches are not lost: a post-pin commit touching a frame's file without touching a frame's symbol is recorded in the dedup evidence as `post_pin_file_touches`, the first five SHAs, and decides nothing. The over-inclusive-by-design argument above stands for the window it was about and not for a single commit that churns an API across the tree, which is what the measurement was; an API-churn commit that does touch a frame symbol still reads as `fixed_post_pin` and is left to the human at the gate, campaign 2's `APInt.h:121` candidate being one.
- **FR-10.5** Every dedup verdict shall carry its evidence: the matching key for a candidate-to-candidate duplicate, the issue number and URL for a known issue, the commit SHA for a post-pin fix. **Amended 2026-09-15 (§1.10): the six verdicts are given a precedence**, which was fixed nowhere although more than one can hold of one candidate. An empty mirror is `dedup_unavailable` for every candidate; then a mirror hit, open before closed; then a candidate-to-candidate duplicate; then a post-pin fix; then `new`. The mirror hit outranks the candidate duplicate because its evidence is the only carrier of FR-13.9's label.
  *AC:* no verdict other than `new` is recorded without a populated evidence field; and a candidate satisfying two rules at once records the higher one, asserted by a test.
- **FR-10.6** Fingerprints shall be persisted and shall be queryable across runs, so a candidate duplicated in a later run is caught. **Amended 2026-09-15 (§1.10):** the query asks for a stored fingerprint whose value matches **and whose `fingerprint_stable` is 1**. That is the only shape in which `duplicate_of_candidate` is reachable: FR-10.1's unstable fingerprints and the pre-re-run nulls sit outside every partition, so no two candidates of one run can merge before the gate has re-run them, which is the design and not an accident.
  ...
  **Erratum 2026-09-15 (D-18): the duplicate is the earliest candidate holding the fingerprint, and it is never its own.** The cross-run query returned **any** other candidate with the value, so two candidates sharing one fingerprint were each the other's duplicate and neither stayed `new`: over the re-screened runs, ten verifier candidates and three invariant assertions collapsed to no representative at all and nothing reached the gate. The query now orders by the candidate's creation time and then its id and takes the **first**, and a candidate that is itself the first is not a duplicate of anything. The `fingerprint_stable = 1` conjunct of the amendment above is unchanged.
...
- **FR-10.7** A GitHub rate-limit or server error shall fail the dedup for that candidate with an explicit `dedup_unavailable` status, and such a candidate shall fail the gate's novelty question rather than passing it.
  *AC:* inject a 403; the candidate is `dedup_unavailable` and F-13 refuses it. Rationale: CHIA raises typed `GithubRateLimitError` (`chia:chia/github/github_client.py:33-39`), so the condition is detectable. **Amended 2026-09-15 (§1.10):** the **refresh's** own failure is recorded on the refresh's return and not in the manifest. `RunManifest.issue_mirror` is closed at six keys and the validator compares the key set exactly, so `issue_mirror_refresh` returns those six plus a seventh, `incomplete_reason`, the caught exception's class name or null, and the driver copies the six.
...
- **FR-10.9** The loop shall mirror `llvm/circt`'s open **and** closed issues into its own database exactly once per run, before the first candidate is screened, using `GithubIssuesNode` with `state="all"` and an **issue cap** `n` `[DEFAULT]` recorded in `budget.yaml`, called as `recent(n=<cap>, fetch_comments=False)`; and it shall record the refresh timestamp, the number of issues mirrored, the cap, whether the cap bound, the state filter, and that comments were not mirrored. ~~The previous cap, "a page cap ... the number of pages actually fetched",~~ is **WITHDRAWN** 2026-09-13 (HLD review W21, W22). The reusable API counts issues, not pages: `recent` takes `n` and `_list` does its own paging internally (`chia:chia/github/github_issues_node.py:139-175`), while `_paginate` (`chia:chia/github/github_client.py:113-128`) is private and is not what `_list` calls, so a page cap could only be honoured by calling a private method, by an unstated pages-to-issues conversion, or by writing a second pager.
  **Amended 2026-09-15 (§1.10, C-22, architect decision 1): the cap is a row bound and the walk is two-directional.** GitHub's issues-listing endpoint refuses page 100, item 10,001, with a **422**, and at the recorded cap a single `desc` walk was measured to write **zero** rows: with a token set, 100 requests in 99.3 s, the first 99 returning 200 and the hundredth 422, caught as a request error so that `incomplete_reason` was set and nothing was stored. The mirror therefore walks `direction=desc` and then, on the ceiling, `direction=asc`, unioning the two deduplicated by issue number. Two directions of 10,000 items each is more numbered items than the repository has issued, its newest issue or pull-request number on 2026-09-14 being **11,113**. A bounded control run at a cap of 600 wrote 600 rows, 249 open and 351 closed, in 35 requests and 33.9 s, with `X-RateLimit-Remaining` 4865 of 5000; 35 listing pages for 600 issues is what dropping pull requests costs, they being the majority of the numbering.
  *AC:* one run mirrors at most `n` issues, all before screening begins; the six recorded values appear in the `RunManifest`; and a second run in the same campaign reuses the mirror unless the operator asks for a refresh. **Added 2026-09-15:** a live run at the campaign's own cap writes a non-zero number of rows, which is the criterion the 422 made necessary. `fetch_comments=False` is required and not merely permitted: the default costs "one extra paginated request per issue that has comments" (`chia:chia/github/github_issues_node.py:67-71`), which over thousands of open and closed issues would spend the authenticated rate limit FR-10.7 only detects, and CHIA's own example passes `False` for the same reason (`chia:examples/circt_issue_solver/triage.py:61`). Two consequences are accepted and disclosed rather than worked around: a known-issue match existing **only** in a comment is missed, which costs a duplicate report; and no maintainer's words enter the database at all, which is what makes FR-20.4's "no automated path exists from a maintainer comment into a prompt" true by construction rather than by inspection. Rationale: `_paginate` fetches 100 per request (`chia:chia/github/github_client.py:113-128`) and `llvm/circt`'s open and closed issues together are thousands, so a per-candidate query would spend the rate limit FR-10.7 only detects. CHIA's own example already sets `TRIAGE_POOL = 2000` for the open backlog alone (`chia:examples/circt_issue_solver/circt_issue_loop.py:50`).
...
- **FR-11.2** The agent's classification shall not override a tool verdict: a candidate the dedup stage marked `known_open_issue`, `known_closed_issue` or `fixed_post_pin` shall be classified `known_issue`, and the agent's only freedom is the reason text.
  ...
  **Erratum 2026-09-15: the stage-6 turn is made only for a `new` candidate.** The duplicate screen runs before the report is written and its verdict is known inside the same node, so for every verdict other than `new` the turn could change nothing at all: this requirement already fixes the classification and FR-13.5 already refuses the candidate at gate question 4. The turn is therefore **skipped** and the report rendered from the record alone, carrying the tool's own classification, `known_issue` for the three issue verdicts, `duplicate` for a candidate-to-candidate duplicate and `untriaged` for `dedup_unavailable`, with a driver-written sentence in the three prose slots saying that no model turn was made and why. No token is charged for such a candidate. FR-11.4 is untouched: every number, size, hash and verdict in that report is read off the record exactly as it is for a candidate that did get a turn. A pilot paid nearly USD 4 for one such turn over a candidate the screen had already matched to an open issue, and bought prose that disagreed with the match and was overruled in the same function.
  **Erratum 2026-09-15 (D-16, D-17): the tool verdict this requirement defers to is now corroborated, and re-screening can change it without a model.** Two mirror matches were measured to be worthless and both forced `known_issue` on candidates that were not one: a **generic** MLIR or LLVM assertion text, one whose site lies under `include/mlir/`, `include/llvm/` or `llvm/Support/`, matched an issue on the assertion text alone, three distinct `StorageUniquerSupport.h:180` candidates all landing on one open issue; and a `verifier_error` candidate matched on the **op name** alone, ten probes landing on two issues, one of them a `good first issue` enhancement that FR-13.9 would then have refused the whole filing over. A generic assertion now matches only where the issue's own text also carries one of the candidate's CIRCT frame tokens, and a verifier candidate only where it carries the op **and** the constraint phrase; the dropped issue number is kept as evidence, `generic_text_match` or `op_only_match`. Because the screen can be corrected after a run, `bug_loop.py --rescreen <run>` recomputes it and gate question 4 for every candidate of a stored run from stored evidence and with **no model turn**, appending a new verdict row rather than overwriting the old one so that both are readable; a candidate that becomes `new` is then owed the stage-6 turn the erratum above skipped, and `--report-turns <run>` makes exactly those.
- **FR-11.3** The `Report` shall contain, at minimum: the reduced case verbatim; the exact reproducing command; the observed behaviour including the assertion text and `file:line` where present; the build identity (CIRCT SHA, SDK tag, image digest) and the `-UNDEBUG` flag with the tool's own `--version` output; the dedup evidence; and the arm that produced it. **Amended 2026-09-15 (§1.10):** the build-identity line carries the literal `-UNDEBUG` in wording that satisfies FR-07.10 for every class, "Built with `-UNDEBUG`, so the compiler's internal checks are on"; and of this requirement's thirteen substitution points, **eleven** can be absent and refused, `contamination` and `assisted_by` being derivable from the record and the manifest in every case, so no incompleteness can fire for them.
...
- **FR-12.5** Repair shall be refused when the oracle marked the root `out_of_scope_root`.
  ...
  **Erratum 2026-09-15: a third refusal, and the two bounds the chain is now given.** Stage 7 runs only for a candidate the duplicate screen of F-10 called **`new`**; every other verdict records `skipped:<verdict>` and invokes nothing, a known or duplicate candidate already being somebody's issue while repair is the most expensive stage of the loop. The one additive branch FR-12.1 permits now passes two further `cfg` keys into the backend it constructs, `max_tool_iterations` from `budget.yaml`'s `stage_7` entry and `turn_budget_usd`, the per-turn money ceiling of FR-14.6's erratum, so no phase of the chain runs with the backend's own defaults and none is unbounded. The chain still dispatches its own turns, so an attempt's tokens stay unobservable exactly as FR-14.6 records; its `LedgerEntry.observed` therefore carries `billed_usd` null and is **authorised, and counted, at five times that ceiling**, one for each phase `run_issue_remote` runs.
...
- **FR-13.2** Question 1 shall be answered by re-running the reproducing command in a **fresh process** with a **newly created working directory**, **on a logical worker other than the one that produced the original verdict where the scheduler can grant it, and recorded either way.** The re-run shall be dispatched as a task of its own, holding the worker resource, by a decision stage that holds no such resource; the preference for another worker shall be expressed as a **soft** node affinity, so that where the preferred worker is busy or gone the re-run still happens and `same_worker=true` is recorded. ~~The previous wording, "on a logical worker different from the one that produced the original verdict whenever the cluster holds more than one worker of that type",~~ is **WITHDRAWN** 2026-09-13 (HLD review K5) as a requirement no schedule can meet: expressed as a hard pin from a resource-holding decision stage, on the two-worker Must configuration, two candidates gating at once each hold one slot and each wait on the other's, and CHIA's own stall detector cannot even see it, because its stuck test requires the resource to be available and with both slots held it is not (`chia:chia/base/chia_wait.py:69-80`, `238-241`).
  ...
  **Erratum 2026-09-15 (D-12): the soft affinity is given a bound, and an unplaced re-run is an answer.** A soft affinity waits for a busy node rather than falling back, which is not what "the re-run still happens" assumed. MEASURED, campaign 2: one gate ran 25 minutes with its re-run in `PENDING_NODE_ASSIGNMENT` while the cluster reported a worker slot in use and no probe running, because each seeded shard's generator held a worker slot it never used for a whole seed iteration; with two shards no re-run could ever be placed and no iteration could end. Two things follow. The seeded generator now declares **no** worker resource, it running no compiler itself, which is the same rule this requirement already applies to the deciding stage. And the re-run is bounded twice and then answered: the preferred worker for 60 s `[DEFAULT]`, then **any** worker of that type for the probe's own wall-clock limit plus 60 s `[DEFAULT]`, then question 1 is answered `rerun_unscheduled` and the candidate is refused. That is a sixth stopping value for question 1 and it buckets `unreproducible` in FR-18.6's table, which is unchanged and stays total; the results artefact prints the stopping value, so a candidate the loop failed to place is distinguishable from one that failed to reproduce. FR-13.15's own check is dispatched the same way and is **not** yet bounded, which is recorded as open.
- **FR-13.3** Question 2 shall be **total**, with an answer for every reducer record F-09 can produce. It passes when a reducer ran to a fixpoint (FR-09.11) and the re-check preserved the failure class, the assertion text and the `file:line`. It fails, as `not_minimal`, when `reduced=false`, when the reduction changed the failure (`reduction_changed_failure`), ~~or when no reducer could run at all~~ **, when no reducer could run at all, or, added 2026-09-15 (§1.10), when a reducer ran and reached no fixpoint, `not_fixpoint`**. The failing reason shall be recorded. The fourth value exists because this requirement's pass demands a fixpoint and FR-18.6's table named only three failing reasons, none of them the one `03-LLD.md` §9.5 records as the common case at the withdrawn 60 s budget; it buckets `not_minimal` with the other three, so the taxonomy is unchanged and stays total. A free-text `ReducedCase.reason`, which FR-09.12 permits, buckets the same way.
  **Erratum 2026-09-15: `already_minimal`, a sixth outcome, and a pass.** A reducer that reached a fixpoint having removed nothing is the reducer stating that the case **cannot** be made smaller, and reading that as `not_minimal` refused every candidate a pilot produced over a six-line case `circt-reduce` had just proved irreducible in twenty interestingness calls. Question 2 now fails only when no fixpoint was reached, `not_fixpoint`, or when the case is **larger than `minimal_case_lines`**, in which case the reducer's own reason is recorded and buckets `not_minimal` as before. A fixpoint with no progress on a case at or below that threshold **passes**, as `already_minimal`; being a pass it never reaches FR-18.6's taxonomy, so the six buckets are unchanged and stay total. A case whose lines cannot be read is not called minimal.
  *AC:* all ~~four~~ ~~five~~ **six** outcomes are exercised by tests, each leaves its reason in the record, and no input to this question is ever null. Before this revision `reduced=false` had no answer here, so FR-13.10 refused those candidates with nothing recorded, and whole probe languages fell into that hole (FR-09.9).
...
- **FR-13.9** A report whose dedup stage matched any `llvm/circt` issue carrying the `good first issue` label shall not be filed, whatever the rest of the gate says; and no filing shall be made as a comment or a fix on such an issue.
  *AC:* a fixture candidate matching a `good first issue`-labelled issue is refused, with that label named as the reason. **Amended 2026-09-15 (§1.10): the refusal is made reachable.** It refuses a filing that matched **any** such issue and the dedup evidence holds the labels of exactly **one**, so under the mirror screen's open-before-closed selection a labelled match could be masked by an unlabelled one and the refusal would silently not fire. The screen promotes a labelled match ahead of the state rule, which is the safe direction for a refusal; a ninth evidence key, the alternative, is a contract MINOR bump this revision declines. A test asserts that a candidate matching one labelled and one unlabelled issue reports the labelled one. Rationale: CIRCT forbids using AI tools on `good first issue` issues (`circt:docs/AIToolPolicy.md:24`); PAPER §4 already commits to leaving them to newcomers.
- **FR-13.10** The gate shall default to `nothing`. A candidate with any unanswered question shall be refused, **and shall be bucketed `undecided`** (added 2026-09-15, §1.10): the default decides the outcome and FR-18.6's table had no row for an unanswered question, so its six values were not total over what the gate can produce.
  *AC:* a candidate with a null answer is refused and bucketed `undecided`, asserted by a test against the pure decision function, the four questions as implemented not being able to return a null on demand.
...
- **FR-13.15** The mechanical validity check answering question 3 shall be: the reduced case parses and passes the tool's own verifier at the run's commit, and the recorded failure occurred after that point. The check command is per input language, fixed here and spelled in full in `03-LLD.md`, and every flag below was verified against the SDK's `--help` on 2026-09-13:
  ...
  *AC:* all three outcomes are exercised, one per input language for the first; the check's exit status and stderr are persisted with the `GateDecision`; and the recorded failure's stage is later than the check's. **Two amendments, 2026-09-15 (§1.10).** The check runs on a **worker node of its own**, `03-LLD.md` §3.9's `gate_validate`, because it is three tool invocations under a probe's `prlimit` prefix against the source-tree binaries and the deciding stage holds no `circt` resource by FR-13.2's design. And the "later than the check's" conjunct needs the failing frame's **full path**, which `OracleVerdict.fingerprint_frame` does not carry, that field being `"<function> <basename>"`; the path is read back out of the recorded frames and re-stripped by the fingerprint's own rule, and "the probe's argv carried a pass pipeline" is read off the recorded argv.
  **Erratum 2026-09-15 (D-13): for a `verifier_error` candidate this check is not run a second time.** That class exists only because the command above was run on the probe's input at stage 3 and exited 0, which is this question's own pass condition, so the gate reads that run's exit status and stderr path off the record rather than dispatching a fourth invocation: the answer would be the same and the worker is scarce. The "later than the check's" conjunct is true by definition for the class, a verifier error being precisely a failure after parsing, the pass having produced an op the verifier then refused. The three commands, the three outcomes and every other class are unchanged.
...
- **FR-14.1** `budget.yaml` shall declare, at minimum: the **primary budget unit** (G-48) as the **arm window** `W` in wall-clock seconds, equal for both arms, together with the **arm order** in which the two windows run; the safety caps, which are generated inputs per day and filings per day and in total; the campaign's start and end timestamps; the corpus HEAD SHA (FR-01.11); the top-N frame count N of G-43; the calibration sample size k of D-01(d); the per-seed probing-input cap of FR-04.5 and the per-seed iteration cap of FR-16.3; the per-probe wall-clock, address-space and CPU-time limits of FR-06.2; the stdout and stderr byte cap of FR-06.4; the reduction wall-clock budget of FR-09.4 and the SIGKILL grace period of FR-09.13; the **issue-mirror issue cap** of FR-10.9; and the filing-poll window of FR-13.16.
  ...
  **Erratum 2026-09-15 (contract 2.2, annotated tag `contract-2.2`).** The file shall declare two further keys, both registered parameters rather than implementation constants. `max_tool_iterations` is a mapping over the four model-bearing stages, `stage_1`, `stage_2`, `stage_6` and `stage_7`, giving the number of backend calls one turn of that stage may make; it is registered because FR-14.6's pre-authorisation is computed from it and the same figure becomes the per-turn money ceiling the backend enforces from the inside, so changing it changes every authorised number. `minimal_case_lines` is FR-13.3's minimality threshold. The registered values, each measured from a pilot rather than chosen: `arm_window_seconds` **7200.0**, two hours an arm and four for the campaign, fixed from the pilot's rate of about USD 0.6 a minute of wall clock against `campaign_spend_cap_usd`, because two four-hour arms at that rate would have been stopped by the money inside the first arm; `max_tool_iterations` **12, 12, 6 and 20**, stages 1 and 2 doubled after a cap of 6 stopped both stage-1 turns of a pilot answering at all; `minimal_case_lines` **12**; and `calibration_sample_size` **0**, with its reason written into the file, no seed parent carrying this deployment's `llvm` pin, so the eligible set is empty and the campaign is registered as discovery mode with no sample. `03-LLD.md` §9.1 carries the two key rows and §9.5 the file.
  **Erratum 2026-09-15 (contract 2.3, annotated tag `contract-2.3`): three recorded fields, and no new registered key.** The ledger entry's `observed` block gains `cached_tokens`, the count of input tokens the backend served from its own context cache, a **subset** of `tokens_in` and never a summand of it; the backend reports it and nothing captured it, so no run could say how much of its input was cached. Its **price** is deliberately not registered: this file declares exactly two rates and the ledger prices every input token at the input rate, so a campaign's reported USD is an **upper bound** and `cached_tokens` is what the overstatement is computed from. The cached rate for the registered model is recorded in code, verified against the same pricing page as the two registered rates, and read by nothing, because a third rate added after the registration commit would not be the file the campaign was registered against (FR-14.7); registering it is a new registration. The `RunManifest` gains `shard`, the `K/N` of the driver's `--shard` or null, because a campaign that drives its corpus from more than one job has to say which part of it each run drove or its counts read as the whole corpus's (FR-18.10). `FeedbackEntry` gains `error_line` in the same bump, which is F-16's.
...
- **FR-14.6** Token and cost usage shall be accrued where the backend reports it. **Tokens and USD shall be observable on the campaign backend** (added 2026-09-14, §1.8): the campaign backend reports per-turn token counts, so every model stage's `LedgerEntry.observed` carries prompt tokens, output tokens and a USD figure, and the results artefact reports the campaign's spend beside the result.
  ...
  **Erratum 2026-09-15 (the spend guard, and what `observed` now carries).** The pre-authorisation that FR-18.10's third stop condition rests on prices the **whole tool loop** and not one call. `llm.SpendGuard.worst_case_usd` sums `prompt_tokens + i * TOOL_OUTPUT_TOKENS_CAP` over `i` in `range(n)` at the input rate and adds `n * max_output_tokens` at the output rate, with `n` being `max_tool_iterations` plus `final_tool_iterations` plus one for a turn that carries tools and one for a turn that does not; the growth is quadratic because the backend re-sends the whole conversation on every call. `TOOL_OUTPUT_TOKENS_CAP` is 32768 and is a cap **by construction** rather than by measurement: one tool return is capped at 65536 bytes by FR-04.4's read-only source tool and the pre-authorisation counts two characters to the token, so no tool result can exceed it. The same authorised figure is handed to the backend as `turn_budget_usd`, a per-turn ceiling checked before every call, which refuses the next call from the inside and is never retried. The guard settles each turn to its bill when usage comes home and keeps the **whole** authorisation for a turn that returned none, which is every stage-7 attempt and every turn that raised, so a dead turn is never priced at nothing. Four figures therefore reach `LedgerEntry.observed` beside the tokens: `authorised_usd`, `ceiling_usd`, `billed_usd` and `calls` (contract 2.2, FR-14.1). `03-LLD.md` §3.5.1 carries the arithmetic.
...
- **FR-18.5** Seeded-bug validation shall be reported separately from discovery, and shall not contribute to the headline. **Amended 2026-09-15 (§1.10):** the table's `probes` column counts a seed's probes across **both** arms, seeds being shared within a mode (FR-18.2), so it is not a per-arm count; that none of this table reaches the headline is the table's own point.
...
- **FR-18.6** The failure taxonomy shall record, for every candidate reaching the gate, exactly one of six values and no other, `unreproducible`, `not_minimal`, `invalid_input`, `duplicate`, `undecided`, `new_bug`; and for every repair attempt, the phase that failed. Every stopping value any gate question can produce maps to exactly one bucket, by this table:
  ...
  | 2 minimal | `not_fixpoint`, a reducer ran and reached no fixpoint (added 2026-09-15, §1.10) | `not_minimal` |
  ...
  | any | the question was not answered at all (added 2026-09-15, §1.10) | `undecided` |
  | none | passed all four | `new_bug` |

  The two rows added 2026-09-15 make the mapping total over what the gate can produce and add **no seventh bucket**; §1.10 records why each was missing. The results artefact prints a **seventh row** in its taxonomy table, `no gate decision`, which is likewise not a bucket: it counts candidates the gate never reached, so a candidate is counted rather than dropped and the sum check stays meaningful.
...
- **FR-18.10** The arms run **sequentially**, each for the same fixed window `W` (G-48), in the order `budget.yaml` records. **Each arm stops when its own window expires, when one of its safety caps binds, or when the campaign's cumulative USD spend reaches `campaign_spend_cap_usd`** (the third condition added 2026-09-14, §1.8), whichever comes first, and the results artefact shall state **both windows and the campaign's spend**: the wall-clock actually elapsed for each arm, and, where a safety cap stopped an arm early, which cap bound and how much of that arm's window went unspent. ~~The previous rule, "the other arm shall stop at the same moment",~~ is **WITHDRAWN** 2026-09-13 (HLD review W1): it presumed the arms overlap in time, which G-48 no longer permits, and under sequential windows an arm that ends early cannot stop an arm that has not started or has already finished. Equality of budget is delivered by the equal window, not by a simultaneous stop; the shortfall disclosure is what keeps the comparison honest when a cap bites first.
  ...
  **Erratum 2026-09-15: an arm may be driven as more than one run, and each run says which part of the corpus it drove.** The seeded arm is embarrassingly parallel over seeds, and the driver's `--shard K/N` keeps the seeds at positions `index % N == K` of the already-fixed corpus order. Each shard is a run of its own with its own window, its own manifest and its own results artefact, and the manifest and section 0 of that artefact both carry the `K/N` string, because a shard's counts are that shard's and not the corpus's and nothing sums them automatically. The equal-window rule above is therefore a rule about the **arm**, and a campaign that shards one arm and not the other has not run the two for equal budget unless it says so beside the headline. MEASURED and recorded here because it bears on the choice: three jobs of one campaign share one rate limit, so two shards do not halve the wall clock; one seed's stage-1 turn took over 20 minutes against about 5 when a single job ran.
...
- **FR-20.1** The method shall be posted to CIRCT's forum before the campaign starts, and the post's URL and date shall be recorded in the `RunManifest`.
  ...
  **Erratum 2026-09-15 (user decision): enforced at the first filing, which is what the criterion above already said.** The requirement's text says "before the campaign starts" and its acceptance criterion says "before the first filing", and the implementation enforced the text: a pre-flight check refused any run whose manifest carried no post. The criterion governs. The pre-flight check now **records** `posted`, `unposted` or `exempt` and refuses nothing, and the approval CLI of FR-13.18 refuses any filing while the run's manifest carries no forum post, naming the two fields; the same command takes the post's URL and date and writes them into the run's manifest before the approval is recorded, so a filing always carries the post it was made under. A run that files nothing, a re-screen or a reclassification among them, therefore needs no post, and the campaign need not wait on one to start.
...
| C-22 | **GitHub's issues-listing endpoint refuses page 100, that is item 10,001, with a `422`.** Added 2026-09-15 (§1.10). The listing is over issues **and** pull requests together, and pull requests are the majority of `llvm/circt`'s numbering | Measured 2026-09-14 with `GITHUB_TOKEN` set (`X-RateLimit-Limit: 5000`): 100 requests in 99.3 s, the first 99 returning 200 and the hundredth 422, caught as `GithubRequestError`, `incomplete_reason` set and **zero** rows written. A bounded control at a cap of 600 returned 600 rows, 249 open and 351 closed, in 35 requests and 33.9 s, `X-RateLimit-Remaining` 4865 of 5000 | FR-10.9's mirror walks `direction=desc` and then, on the ceiling, `direction=asc`, unioning the two deduplicated by number; the cap stays the **row** bound. A repository with more than 20,000 numbered items cannot be fully mirrored this way and would need a different retrieval, which `llvm/circt` does not, its newest number on 2026-09-14 being 11,113 |
...
|---|---|---|---|
| A-01 | An agent can turn a CIRCT fix diff into productive sibling hypotheses | **Unverified** | Assumed from AFuzz's V8 and SpiderMonkey/JSC results; never measured on CIRCT. Verify: F-04's pilot over 5 seeds, counting probing inputs that fire the primary oracle. **Dated note, 2026-09-15 (§1.10)**, which does not verify it: the apparatus the pilot needs now exists and is measured. Six recorded crash fixtures are committed, `crash_01`, `assertion_01` to `assertion_04` and `fatal_error_01`, across two SDK tags, 1.143.0 and 1.156.0; `_ASSERT_UNREACHABLE` is still unexercised on real output; and calibration-mode build costs are **14 s to 601 s per parent build**, which is what a calibration run's own budget has to carry |
...
| A-05 | The three dedup rules separate duplicates from distinct bugs | **Closed** 2026-09-15 (§1.10), with a stated caveat | Measured over FR-10.2's labelled set, which is **22** pairs and not 20, eleven duplicate and eleven distinct, at least four per construction rule and five R4: **collision rate 1/11 = 0.0909, false-merge rate 1/11 = 0.0909**. Two of the pairs are the point of the exercise and neither is synthesised: one is `assertion_01` against `assertion_04` as the latter would have been recorded under the former's SDK, where the generic `cast<>` check sits at the same `Casting.h` line, and the other is the ten-run `!hw.array` measurement whose fingerprint frame moved between three adjacent frames of one recursion cycle. **The caveat is printed beside the rates**: the set is constructed from six recorded failures and not from a pilot's own probes, which `04-Test-Plan.md` §10 asks for and which no run has yet produced |
...
| A-08 | Both differential harness generators are feasible from a port list | **Closed by measurement** 2026-09-15 (§1.10) | The SDK ships no `arcilator-header-cpp.py` (verified: zero `find` hits) and its `circt-test-runner-verilator.py` expects a port convention a generated design will not satisfy, so both generators were written. Measured end to end through `oracle_differential`: a generated clocked accumulator agrees over all **56** sampled cycles, and the same design with one constant changed diverges at line index 1, cycle 8, signal `p`, `1` against `0`. `03-LLD.md` §4.13's `[UNVERIFIED]` row and its §15 item 1 close with this |
...
| A-19 | The differential can discriminate a useful share of the corpus | **Still open, and now unblocked** (§1.10) | No count exists for FR-08.1's applicability rule, and the 11 that RT3 §1 K2 reported answered a different question under a different bucketing. **As of 2026-09-15 the rule exists as a pure function** (`probe_task.differential_applicable`, FR-08.1 as amended), so the obstacle is no longer that there is nothing to run; the count over the 187-seed corpus has not been run. Verify: run it and report the count with the rule. `[UNVERIFIED]` |
