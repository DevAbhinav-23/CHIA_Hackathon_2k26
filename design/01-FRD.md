...
- **FR-13.3** Question 2 shall be **total**, with an answer for every reducer record F-09 can produce. It passes when a reducer ran to a fixpoint (FR-09.11) and the re-check preserved the failure class, the assertion text and the `file:line`. It fails, as `not_minimal`, when `reduced=false`, when the reduction changed the failure (`reduction_changed_failure`), ~~or when no reducer could run at all~~ **, when no reducer could run at all, or, added 2026-09-15 (§1.10), when a reducer ran and reached no fixpoint, `not_fixpoint`**. The failing reason shall be recorded. The fourth value exists because this requirement's pass demands a fixpoint and FR-18.6's table named only three failing reasons, none of them the one `03-LLD.md` §9.5 records as the common case at the withdrawn 60 s budget; it buckets `not_minimal` with the other three, so the taxonomy is unchanged and stays total. A free-text `ReducedCase.reason`, which FR-09.12 permits, buckets the same way.
  *AC:* all ~~four~~ **five** outcomes are exercised by tests, each leaves its reason in the record, and no input to this question is ever null. Before this revision `reduced=false` had no answer here, so FR-13.10 refused those candidates with nothing recorded, and whole probe languages fell into that hole (FR-09.9).
