...
**Resolved by the W-04 image build (2026-09-14).** Branch (a) holds in the real image: `chia-circt-assert:eade0de61bc5` contains `circt-verilog` (`CIRCT eade0de`, `slang version 11.0.385+44dc55f99`) and `circt-translate --import-verilog`; the 61 `REQUIRES: slang` lit tests that M3 skipped now run and pass. No seed is excluded; `RunManifest.sv_seeds_excluded` is null in discovery mode. Cost inside the image: the six-target build took 972 s at -j8 with `-DLLVM_PARALLEL_LINK_JOBS=2`; see `analysis/measurements/2026-09-14-image-build.md`.
