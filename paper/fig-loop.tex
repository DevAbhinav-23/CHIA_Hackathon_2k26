...
% Absolute coordinates in cm. Canvas: x 0.00..18.10, y 1.10..11.15.
% The ribbon's top edge is constant at y=5.50. Its thickness drops at each gate
% and the lost slice peels away downward onto the floor at y=1.95.
...
\fill[agF, draw=agB, line width=0.5pt]
  (4.80,8.32) -- (5.40,8.32) -- (5.55,5.50) -- (4.15,5.50) -- cycle;
% shaft: the flow rises into stage 6 and comes back down
\fill[agF] (12.05,5.50) rectangle (13.10,8.32);

% ================================================================== agent band
\draw[fill=agF, draw=agB, line width=0.5pt, rounded corners=2pt]
  (1.60,8.32) rectangle (13.75,11.15);
\node[anchor=base west] at (1.80,10.88)
  {{\color{agB}\hdr{AGENT}}\;{\sffamily\fontsize{6.2}{7.2}\selectfont
   \color{dgrey}proposes and writes; judges nothing}};

\node[card, text width=2.24cm, minimum height=2.20cm] at (1.75,10.72)
  {\ct{\nm{1}{agB}\; Read the seed}\cd{States the root-cause class the fix
   corrected and names sibling sites.}};
\node[card, text width=2.96cm, minimum height=2.20cm] at (4.52,10.72)
  {\ct{\nm{2}{agB}\; Write probing inputs}\cd{For those sites, in the seed's own language.}\cm{circt-opt}\cm{firtool}};
\node[card, text width=2.58cm, minimum height=2.20cm] at (10.78,10.72)
  {\ct{\nm{6}{agB}\; Triage and report}\cd{Groups duplicates, checks the issue
   tracker, writes the report.}};

\draw[arr] (4.25,9.62) -- (4.50,9.62);

\node[anchor=center, text width=2.42cm, align=center, color=agB,
      font=\sffamily\itshape\fontsize{6.2}{7.4}\selectfont] at (9.25,9.62)
...
\foreach \i in {0,1,2}
  {\draw[fill=white, draw=dsB, line width=0.5pt, rounded corners=2pt]
     (0.05+0.09*\i,9.13+0.10*\i) rectangle +(1.20,1.42);}
\node[txt, text width=0.98cm] at (0.33,10.66)
  {\cd{A past CIRCT fix: diff and test}};
\node[txt, text width=1.62cm] at (0.05,9.00)
  {\cd{\textbf{Seed corpus:} 187 mined CIRCT fixes}};
\draw[arr] (1.47,9.90) -- (1.73,9.90);

% ================================================================= return rail
\draw[rail] (13.57,7.62) -- (3.30,7.62);
\draw[rarr] (3.30,7.62) -- (3.30,8.32);
...
\draw[arr, preaction={draw, line width=2.2pt, agF}] (12.35,5.52) -- (12.35,8.30);
\draw[arr, preaction={draw, line width=2.2pt, agF}] (12.80,8.30) -- (12.80,5.52);
...
\foreach \g/\bo/\bn/\q in {7.55/3.95/4.16/builds?, 9.55/4.16/5.00/fails?,
  ...
  {\fill[dsF, draw=dsB, line width=0.5pt]
     (\g,\bn) .. controls (\g+0.50,\bn) and (\g+0.45,1.95+\bn-\bo) ..
     (\g+0.95,1.95+\bn-\bo) -- (\g+1.80,1.95+\bn-\bo) -- (\g+1.80,1.95)
     -- (\g+0.95,1.95) .. controls (\g+0.45,1.95) and (\g+0.50,\bo) ..
     (\g,\bo) -- cycle;
...
\foreach \g/\l in {7.55/does not build, 9.55/no failure,
  ...
  {\node[txt, text width=1.50cm] at (\g+0.95,1.85)
     {\cd{\l}};}
...
\fx{0.05}{7.02}{Build point}{CIRCT at release-pinned main, prebuilt SDK}
\fx{0.05}{6.07}{Build image}{CIRCT's own assertions compiled back in}
\fx{0.05}{5.12}{Issue tracker}{open and closed issues, read only}
\fx{0.05}{4.17}{Budget file}{compute, input and filing caps, set before the run}

% ======================================================================== floor
\draw[hair] (0.05,1.95) -- (18.05,1.95);
\node[anchor=base west, color=dgrey] at (0.05,2.80) {\hdr{WHAT THE GATES DISCARD}};
\draw[hair] (0.05,2.66) -- (3.65,2.66);
\node[txt, text width=3.60cm] at (0.05,2.52)
...
\node[anchor=north, text width=2.22cm, align=center, color=white] at (16.80,5.66)
  {{\sffamily\fontsize{6}{6.9}\selectfont A named author approves each filing,
   under a cap on filings per day. Nothing is filed by default.\par}};

\draw[arr, draw=hgB] (16.80,3.95) -- (16.80,2.12);
\fill[hgB] (16.20,1.95) rectangle (17.45,2.05);
\node[txt, text width=1.85cm] at (16.20,1.85)
