...
def test_T_U_gen_21_the_interlock_refuses_without_the_variable(monkeypatch):
    ...
    assert generate_task._LIVE_MODEL_ENV not in os.environ
    with pytest.raises(LiveModelRefused) as raised:
        build_llm("system", 2400, MODEL_ID)
    assert generate_task._LIVE_MODEL_ENV in str(raised.value)
    ...
    assert generate_task.require_live_model("test") == SYNTHETIC_KEY
