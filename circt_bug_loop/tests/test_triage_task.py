"""`04-Test-Plan.md` §1.11: `triage_task.py` (B6a), the fingerprint and the partition, the mirror and the two screens, the triage turn and the render."""
...
# The throwaway git repository and the no-Ray tool construction are shared with the tool tests rather than built twice: T-U-triage-36 constructs the real `SourceReadTool`.
...
#: Tier 0 for the whole module, and one warning filter.
...
#: FR-10.2's labelled set, read from its PACKAGE HOME.
...
def _store(tmp_path, *, candidate=None):
    """A `LoopStore` on a real file, with the run, probe and build rows a candidate's foreign keys need."""
...
def test_triage_01_assertion_fingerprint_keeps_the_message_tail():
    """T-U-triage-01 (FR-10.1): the assertion fingerprint is expr, newline, site, whitespace collapsed and the ends stripped, and the `&& "message"` tail kept."""
...
def test_triage_02_site_normalisation_survives_two_build_directories():
    """T-U-triage-02 (FR-10.1): the build prefix goes, any other absolute path keeps its last two components, the line number is verbatim."""
    cases = {c["id"]: c for c in
             json.loads((TRIAGE / "fingerprint/assertion.json").read_text())}
...
def test_triage_03_crash_fingerprint_is_the_signal_and_one_frame():
    """T-U-triage-03 (FR-10.1): signal plus the first stripped CIRCT frame, not the top-N tuple, and §3.7.1's measurement is asserted as a property of the labelled set rather than quoted."""
...
def test_triage_04_frame_name_normalisation_is_the_four_steps():
    """T-U-triage-04 (FR-10.1): take FunctionName, cut at the first `(` at bracket depth zero, strip a trailing ` const`, collapse whitespace."""
...
def test_triage_05_insufficient_basis_deduplicates_against_nothing(tmp_path):
    """T-U-triage-05 (FR-10.8): no assertion text, no qualifying CIRCT frame and fewer than N resolving frames is `insufficient`, value None, merged with nothing; the structural hash is still recorded."""
...
def test_triage_06_structural_hash_is_evidence_and_never_a_merge_key():
    """T-U-triage-06 (FR-10.1): SSA names, `@symbol` names, `loc(...)` and whitespace runs are normalised, and no merge path reads the hash."""
...
def test_triage_07_duplicate_relation_is_an_equivalence_relation():
    """T-U-triage-07 (FR-10.2): reflexive, symmetric and transitive over every ordered triple of the labelled pair set of §10."""
    items = []
    for pair in PAIRS:
...
def test_triage_08_partition_does_not_depend_on_candidate_order():
    """T-U-triage-08 (FR-10.2): 100 shuffles of the candidate list give one partition, compared as a set of frozensets."""
    items = []
    for pair in PAIRS:
        for side in ("a", "b"):
...
def test_triage_09_measured_collision_and_false_merge_rates(capsys):
    """T-U-triage-09 (FR-10.2): both rates are computed and printed; the test asserts they are reported, not that either meets a threshold."""
    pairs, by_rule = [], {}
    for raw in PAIRS:
...
def test_triage_25_the_strip_precedes_every_use_of_a_frame():
    """T-U-triage-25 (FR-10.1): the tuple evidence is taken after the prologue strip, and a degenerate name is never the fingerprint's."""
...
def test_triage_26_unstable_fingerprints_are_reported_and_never_merged():
    """T-U-triage-26 (FR-10.1): `fingerprint_stable=false` is its own singleton, and None, the state before the gate's re-run, is likewise."""
...
def test_triage_27_the_two_fallbacks_and_the_evidence_tuple():
    """T-U-triage-27 (FR-10.1): no qualifying CIRCT frame falls back to the top-N tuple; fewer than N resolving frames is `insufficient`; the tuple is recorded as evidence in every case, the `assertion` basis included."""
...
def test_triage_10_the_one_mirror_call_never_fetches_comments(tmp_path, monkeypatch):
    """T-U-triage-10 (FR-10.9): one `state="all"` listing, `n=issue_cap`, `fetch_comments=False`, asserted on the recorded call."""
...
def test_triage_11_no_comment_text_reaches_loop_db(tmp_path, monkeypatch):
    """T-U-triage-11 (FR-10.9): a recorded response carrying comments leaves none in `loop.db`, because none is fetched and none is stored."""
...
def test_triage_12_all_six_manifest_values_are_recorded(tmp_path, monkeypatch):
    """T-U-triage-12 (FR-10.9): the six values of `RunManifest.issue_mirror`, `comments_mirrored` always false, and they validate inside a manifest."""
    payload = json.loads((MIRROR / "sample.json").read_text())
    _recording_transport(monkeypatch, payload)
    token = tmp_path / "token"
    token.write_text(FAKE_TOKEN)
...
def test_triage_37_the_walk_is_two_directional_and_unions_by_number(tmp_path,
                                                                    monkeypatch):
    """T-U-triage-37 (FR-10.9): `desc` to the ceiling, then `asc`."""
    ...
    # Two directions were asked for, and the refused page was asked for once in each.
...
def test_triage_37b_the_cap_bounds_both_directions(tmp_path, monkeypatch):
    """T-U-triage-37 (FR-10.9): `issue_mirror_issue_cap` is the row bound of the UNION and not of one direction, so the second walk stops at it too."""
...
def test_triage_38_only_a_422_is_the_ceiling(tmp_path, monkeypatch):
    """T-U-triage-38 (FR-10.7): another 4xx is a failure, not a ceiling."""
...
def test_triage_13_a_rate_limit_leaves_a_partial_mirror(tmp_path, monkeypatch):
    """T-U-triage-13 (FR-10.7): a `GithubRateLimitError` is caught, the count reached is recorded, screening proceeds against the partial mirror."""
...
def test_triage_14_an_empty_mirror_is_dedup_unavailable(tmp_path, clone):
    """T-U-triage-14 (FR-10.7): an empty mirror yields `dedup_unavailable` for every candidate, which fails gate question 4 rather than passing it."""
...
def test_triage_15_screening_makes_no_github_request(tmp_path, monkeypatch):
    """T-U-triage-15 (FR-10.3): a whole screening pass over 50 candidates shows zero requests to api.github.com; the mirror is the only source."""
    import requests
...
def test_triage_16_a_known_closed_issue_comes_back_with_its_number(tmp_path):
    """T-U-triage-16 (FR-10.3): F-10's own acceptance, against the recorded mirror."""
    store = _store(tmp_path)
    payload = json.loads((MIRROR / "sample.json").read_text())
    _mirror_rows(store, [{
...
def test_triage_17_every_verdict_but_new_carries_evidence(tmp_path, clone):
    """T-U-triage-17 (FR-10.5): `dedup_evidence` holds exactly the eight declared keys and no issue text, and the per-verdict required subset is populated; `store.validate_candidate` is the enforcement point."""
    candidate = _candidate(tmp_path, screened=True, run_commit=clone["c1"])
    store = _store(tmp_path, candidate=candidate)
    _mirror_rows(store, json.loads((DEDUP / "known_issue/issues.json").read_text()))
    verdict = _verdict(
...
def test_triage_19_the_contamination_scan_is_symbol_level(tmp_path, clone):
    r"""T-U-triage-19 (FR-15.1): symbol level, read from git's `@@ ... @@` hunk-header context and never from the crude `\\b(\\w+)\\s*\\(` rule."""
...
def test_triage_20_both_flags_and_the_bound_are_recorded(tmp_path, clone):
    """T-U-triage-20 (FR-15.1): both flags are recorded, and `contamination_lower_bound` names the run's commit for a non-exact seed, so all 187 stay screenable."""
...
def test_triage_28_the_token_set_is_one_case_per_oracle_class():
    """T-U-triage-28 (FR-10.3): `assertion` two tokens, `crash` one, `fatal_error` two, `differential` none."""
...
def test_triage_29_a_short_token_is_dropped_before_the_query_runs(tmp_path):
    """T-U-triage-29 (FR-10.3): shorter than `MIRROR_TOKEN_MIN_CHARS` goes, so `fold` alone cannot answer `known_issue` for every candidate."""
...
def test_triage_30_open_beats_closed_and_the_evidence_names_the_match(tmp_path):
    """T-U-triage-30 (FR-10.3): one query per token, case-sensitive, any token matching any issue is a hit, and the open issue wins."""
    store = _store(tmp_path)
...
def test_triage_30b_a_good_first_issue_match_is_the_one_reported(tmp_path):
    """FR-13.9 and C-05: a candidate matching an issue carrying `good first issue` reports that issue, so the label reaches the approver's refusal through `DedupVerdict.evidence["issue_labels"]`."""
    store = _store(tmp_path)
    _mirror_rows(store, json.loads((DEDUP / "known_issue/issues.json").read_text()))
...
def test_triage_31_no_match_is_not_a_verdict(tmp_path, clone):
    """T-U-triage-31 (FR-10.3): a clean miss leaves the screen contributing nothing, and the candidate may be `new`; the screen does not invent `dedup_unavailable` for a quiet week."""
...
def test_triage_32_the_scans_bound_on_the_candidates_own_run_commit(tmp_path, clone):
    """T-U-triage-32 (FR-02.7): built against a calibration manifest carrying two entries, a candidate from the second seed scans from its own commit; index 0 is another seed's and is this one's by accident."""
    manifest = _manifest()
...
def test_triage_33_b6b_is_a_head_node():
    """T-U-triage-33 (FR-10.3): `dedup_and_screen` declares no `circt` resource and takes `clone_path` as a parameter."""
...
def test_triage_21_the_tool_verdict_wins(tmp_path, monkeypatch):
    """T-U-triage-21 (FR-11.2): a candidate the dedup stage marked known is classified `known_issue`, and since W-18b (errata row 47) NO TURN IS MADE for it at all: the tool verdict already decides the classification and gate question 4 already refuses the candidate, so there is nothing an agent could change and a whole tool loop to pay for."""
...
def test_triage_22_the_reason_is_truncated_not_rejected(tmp_path, monkeypatch):
    """T-U-triage-22 (FR-11.1): capped at four sentences, counted on `.`, `!` and `?` followed by whitespace or the end, truncated with the truncation recorded."""
...
def test_triage_23_every_substitution_point_equals_the_record(tmp_path):
    """T-U-triage-23 (FR-11.3): the thirteen points are present and equal the record field by field, and no number is the agent's."""
    candidate, reduced, verdict, dedup = _render_bundle(tmp_path)
    manifest = _manifest()
    ...
    # A missing point fails the render, one point at a time.
    removals = {
...
def test_triage_24_template_selection_by_candidate_class(tmp_path, monkeypatch):
    """T-U-triage-24 (FR-07.10): a `differential` candidate takes the second template, a `fatal_error` quotes its own message and says neither of the two forbidden words, and the trailer is the last line."""
    manifest = _manifest()
...
def test_triage_34_two_templates_two_lists_and_no_leaked_dollar(tmp_path):
    """T-U-triage-34 (FR-08.6): the thirteen and the twelve are enforced separately, and neither filling of the prompt leaves a `$name`."""
    manifest = _manifest()
    candidate, reduced, verdict, dedup = _render_bundle(tmp_path)
...
def test_triage_footer_parser_takes_the_last_block():
    """§7.1: the last fenced json block wins, and every other outcome is one of the four `PromptContractError` reasons."""
...
def test_triage_turn_failure_yields_untriaged_and_an_empty_report(tmp_path,
                                                                  monkeypatch):
    """FR-11.8, FR-11.7: a failed or unparseable turn yields `untriaged` and an empty report, and the raw output is persisted whatever happens."""
    candidate, reduced, verdict, dedup = _render_bundle(tmp_path)
...
def test_triage_report_records_the_render_and_refuses_a_differential_dedup(tmp_path,
                                                                          monkeypatch):
    """FR-11.3 and FR-08.10: the `Report` record carries the render's own hash and its point list, and a `differential` candidate never reaches B6b."""
...
def test_triage_36_the_source_read_tool_is_constructed_for_real(tmp_path, monkeypatch,
    ...
    """T-U-triage-36 (FR-04.4): B7 builds §3.5's real `SourceReadTool`."""
    ...
    # The tool answers for real at the run's commit.
    assert tool.read_file("README.md").startswith("circt")
    assert tool.read_file("NoSuch.cpp").startswith("Error:")
    # Started and stopped: one server was constructed and the registry is empty again.
...
def test_triage_frame_paths_and_symbols_are_repo_relative():
    """§3.7.2: a frame path under a CIRCT source root becomes the repo path, and anything else falls back to a basename glob; the symbols carry both spellings git writes into a hunk header."""
...
def test_triage_35_live_mirror_against_llvm_circt(tmp_path):
    """T-U-triage-35 (FR-10.9), LIVE and `t1`: the TWO-DIRECTION listing of `llvm/circt`, open and closed, pull requests excluded as the node does, up to `budget.yaml`'s `issue_mirror_issue_cap`, recording the refresh time, the page count and whether GitHub's pagination ceiling was reached."""
    import time
